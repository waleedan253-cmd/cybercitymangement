...
DROP TABLE IF EXISTS public.laptop_ranges CASCADE;
...
-- Create laptops table (FIXED - only one foreign key constraint)
CREATE TABLE IF NOT EXISTS public.laptops (
  ...
  CONSTRAINT fk_range FOREIGN KEY (range_id) 
    REFERENCES public.laptop_ranges(id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_laptop_ranges_created_at ON public.laptop_ranges(created_at DESC);
...
DROP TRIGGER IF EXISTS set_updated_at ON public.laptop_ranges;
...
DROP POLICY IF EXISTS "Enable read access for all users" ON public.laptop_ranges;
DROP POLICY IF EXISTS "Enable read access for all users" ON public.laptops;
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON public.laptop_ranges;
DROP POLICY IF EXISTS "Enable update for authenticated users" ON public.laptop_ranges;
DROP POLICY IF EXISTS "Enable delete for authenticated users" ON public.laptop_ranges;
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON public.laptops;
DROP POLICY IF EXISTS "Enable update for authenticated users" ON public.laptops;
DROP POLICY IF EXISTS "Enable delete for authenticated users" ON public.laptops;
...
CREATE POLICY "Enable read access for all laptops" 
  ON public.laptops FOR SELECT 
  USING (true);

-- Create policies for write access (using anon key for simplicity)
...
CREATE POLICY "Enable insert for authenticated laptops" 
  ON public.laptops FOR INSERT 
  WITH CHECK (true);

CREATE POLICY "Enable update for authenticated laptops" 
  ON public.laptops FOR UPDATE 
  USING (true);

CREATE POLICY "Enable delete for authenticated laptops" 
...
COMMENT ON COLUMN public.laptops.range_id IS 'Foreign key to laptop_ranges table';
COMMENT ON COLUMN public.laptops.upload_order IS 'Order in which images were uploaded for display purposes';
...
WHERE tc.table_name IN ('laptops', 'laptop_ranges')
